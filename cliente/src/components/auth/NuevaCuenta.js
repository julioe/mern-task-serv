...
const NuevaCuenta = (props) => {
  ...
  // En caso de que el usuario se haya autenticado o registrado o sea un segistro duplicado
  useEffect(() => {
    if (autenticado) {
  ...
  const onSubmit = (e) => {
    ...
    if (password.length < 6) {
      mostrarAlerta(
        "El pasword debe ser de al menos 6 caracteres",
        "alerta-error"
      );
      return;
    }
    ...
    if (password !== confirmar) {
      mostrarAlerta("Los pasword no son iguales", "alerta-error");
      return;
    }
